...
export default function ReneQuantGrid({ rootNote, name, type, onClick }: { rootNote: string, name: string, type: string, onClick: (e: React.MouseEvent) => void }) {
...
function Step({ count, note, active, root }: { count: number, note: string, active: boolean, root: string }) {
    return (
        <div className={cn("relative flex flex-row items-center justify-center gap-2 h-full w-full p-0.5 border-2 border-neutral-900 text-xs font-mono text-neutral-200", active ? "bg-red-500" : "bg-neutral-300/10")}>
